...
describe('Input Component', () => {
    ...
    test('applies custom size class', () => {
        render(React.createElement(Input, { customsize: "large", placeholder: "Large Input" }));
        const inputElement = screen.getByPlaceholderText(/Large Input/i);
        expect(inputElement).toHaveClass('input large');
    });
    test('is disabled when disabled prop is true', () => {
        render(React.createElement(Input, { disabled: true, placeholder: "Disabled Input" }));
    ...
    test('supports other input attributes', () => {
        ...
        render(React.createElement(Input, { type: "text", value: "Test Value", onChange: handleChange }));
